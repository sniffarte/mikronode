...
var api = function(host, user, password, options) {
    ...
    if (!options.port) {
        this.port = options.tls ? 8729:8728;
    } else {
        this.port=options.port;
    }
    this.tls = options.tls ? true:false;
...
api.prototype.connect=function(connectHandler) {
    if (this.tls) {
        this.socket = tls.connect({ port: this.port, host: this.host, rejectUnauthorized: false }, connectHandler);
    ...
    this.socket.on('error', function(a){
        ...
        self.handler.emit('timeout',a,this.handler);
        self.handler.emit('error',a,this.handler);
        self.handler.emit('close',this.handler);
        self.handler.removeAllListeners();
    });
    /* Hack to stop crash if connection times out */
    this.on('timeout', function(a){
        if (self.debug>1) util.log('Connection error: '+a);
           self.socket.destroy();
           self.connected=false;
           delete api._conn[this.hash];
    ...
    this.socket.setTimeout(this.timeout*1000, function(e) { // the socket timed out. According to the NodeJS api docs, right after this, it will be._closed.
        if (self.debug) util.debug('Socket Timeout');
        //if (!self.connected) self.emit('error','Timeout Connecting to host');
        if (!self.connected) self.emit('timeout', 'Timeout Connecting to host');
    });
